import os
from src.inpaint.ssim_loss import ssim
import torch
...
#Added
from src.utilities.utility_pro import calculate_au_pro
...
def test_epoch_anomaly_maps(anomaly_maps,gt_mask_list,gt_list, test_task_index, run, class_name,index_training,path_logs):
    ...
    scores = anomaly_maps
    if isinstance(test_task_index, np.ndarray):
        img_scores = test_task_index.copy()
        print("correct choice")
        print(f"img_score shape: {img_scores.shape}")
    else:
    ...
    threshold1 = thresholds[np.argmax(f1)]
    ...
    best_f1_img = np.max(f1)

    #au-pro score
    predictions = np.asarray(scores)
    predictions = predictions.tolist()
    predictions = [np.array(sample) for sample in predictions]

    gts = gt_mask.squeeze()
    gts = gts.tolist()
    gts = [np.array(sample) for sample in gts]

    au_pro_score,_ = calculate_au_pro(gts, predictions, integration_limit=0.3, num_thresholds=100)


    diz = {"per_pixel_rocauc":per_pixel_rocauc, "img_roc_auc":img_roc_auc, "threshold":threshold1, "pr_auc_score":pr_auc_score, "f1":best_f1, "f1_img":best_f1_img,  "au_pro_score": au_pro_score  }
...
def test_anomaly_maps(anomaly_maps,gt_mask_list,gt_list, test_task_index, run, class_name,index_training,path_logs):
    ...
    per_pixel_rocauc = sklearn.metrics.roc_auc_score(gt_mask.flatten(), scores.flatten())
    # plot ROC curve
    dir_path = os.path.join( path_logs, f"T{index_training}", "pictures_roc").replace('\\','/')
    os.makedirs(dir_path, exist_ok=True)
    filepath = os.path.join(dir_path, str(test_task_index) + '_roc_curve.png').replace('\\','/')
    ...
    print("pictures")
    dir_path = os.path.join( path_logs, f"T{index_training}", "pictures_f1s").replace('\\','/')
    os.makedirs(dir_path, exist_ok=True)
    filepath = os.path.join(dir_path, str(test_task_index) + f'_f1s.png').replace('\\','/')
    plot_values_thresholds(f1[:-1], thresholds, "f1s", filepath)

    dir_path = os.path.join( path_logs, f"T{index_training}", "pictures_pr_curve").replace('\\','/')
    os.makedirs(dir_path, exist_ok=True)
    filepath = os.path.join(dir_path, str(test_task_index) + '_pr_curve.png').replace('\\','/')
...
def reconstruct_epoch_with_evaluation_ad(strategy,args, data_loader, complete_dataset,class_name,index_training,test_task_index,run,path_logs):
    ...
    if strategy.save_embedding:
        ...
        if dataset_path_test!="" and type_folder_test!="":
            ...
            path_originale = os.path.join(dataset_path_test, type_folder_test, f"T9", f"{test_task_index}" ).replace('\\','/')
            if memory_reconstruct_dataset_path_test!="" and type_memory_reconstruct_test!="":
                path_dst = os.path.join(memory_reconstruct_dataset_path_test,  type_memory_reconstruct_test, f"T{index_training}", f"{test_task_index}").replace('\\','/')
            else:
                path_dst = os.path.join(path_logs,  "evaluate_data", f"T{index_training}", f"{test_task_index}").replace('\\','/')
            ...
            filepath_allowed_indices = os.path.join(path_logs,filename_allowed_indices).replace('\\','/')
            save_pickle(allowed_indices, filepath_allowed_indices)

            output_file = os.path.join(strategy.path_logs, f"output_fid.txt").replace('\\','/')
...
def produce_embedding(strategy,args,mode,lista_indices, lista_labels,lista_anomaly_info,lista_filepaths, recon_imgs,test_task_index, run, class_name,index_training,path_logs,original_imgs):
    ...
    for i in tqdm(list(range(len(lista_indices)))):
        ...
        sample_id = i
        dir_path = os.path.join( path_logs, f"{mode}", f"T{index_training}",  f"{class_id}" ).replace('\\','/')
        os.makedirs(dir_path,exist_ok=True)
        ...
        filepath = os.path.join(dir_path, f"{index_dataset}_embedding.pickle").replace('\\','/')
        ...
        features = get_cnn_features(feature_extractor, input_tensor, strategy.device)
        features = [f.cpu() for f in features]
        filepath = os.path.join(dir_path, f"{index_dataset}_original_embedding.pickle").replace('\\','/')
        save_pickle(features,filepath) 
...
def test_results(args, scores, mses, gms_losses, ssim_losses, gt_list,gt_mask_list, recon_imgs, test_task_index, run, class_name,index_training,path_logs,test_imgs):
    ...
    for metric_name in metrics_values:
        dir_path = os.path.join( path_logs, f"T{index_training}", "pictures_thresholds").replace('\\','/')
        os.makedirs(dir_path, exist_ok=True)
        filepath = os.path.join(dir_path, str(test_task_index) + f'_{metric_name}.png').replace('\\','/')
...
def forgetting(metrics_test, T):
    result = 0.0  # Initialize the result variable
    if T != 0:
        for j in range(T):
            max_value = 0.0  # Initialize the max_value for each iteration
            for l in range(T):
                numerator = metrics_test[T][j]["f1"]
                denominator = metrics_test[l][j]["f1"]
                ratio = 1 - numerator / denominator
                max_value = max(max_value, ratio)
            result += max_value
        result /= T
    print(f'Forgetting for task {T} is: {result}')
    return result