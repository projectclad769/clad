...
from src.models.draem_add.loss import *
import numpy as np
...
def create_draem(strategy, img_shape, parameters):
    device = torch.device("cuda:0") if torch.cuda.is_available() else torch.device("cpu")
    draem = DRAEM(device)
    draem = draem.to(device)
    return draem, device  

def weights_init(m):#in draem.py in src/models
...
class DRAEM(nn.Module):
    ...
    def forward(self, gray_batch, aug_gray_batch, anomaly_mask):
        self.model.train()
        self.model_seg.train()
    ...
    def model_test(self, gray_batch):
        self.model.eval()
        self.model_seg.eval()
        ...
        with torch.no_grad():
            ...
            out_mask_cv = out_mask_sm[: ,1 ,: ,:].detach().cpu().numpy()#(1,256,256)
